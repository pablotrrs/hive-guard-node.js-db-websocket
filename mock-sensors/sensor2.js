...
if (process.env.IS_DOCKER_COMPOSE === 'true') {
  console.log('Mock sensor data streamer 2 running in docker-compose mode')
  exec('nslookup server', (error, stdout, stderr) => {
    ...
    connectWithMaster_AndSendDataOver(process.env.WS_PORT, process.env.CLIENT_HTTP_PORT, process.env.CLIENT_UDP_PORT, true)
  });
} else {
  const ip = '127.0.0.1';
  const wsPort = 8002;
